...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation {
    GMSMarker* current = [GMSMarker markerWithPosition:CLLocationCoordinate2DMake(newLocation.coordinate.latitude, newLocation.coordinate.longitude)];
    [path addCoordinate: current.position];
 ...
 * @return void
 */
- (void)setMarkersSet:(NSDictionary *)markers
{
    [self clear];
    path = [GMSMutablePath path];
    for (id marker in markers) {
        NSString *publicId = marker;
        NSDictionary *array = [markers objectForKey: marker];

        CLLocationDegrees latitude = ((NSNumber*)array[@"latitude"]).doubleValue;
        CLLocationDegrees longitude = ((NSNumber*)array[@"longitude"]).doubleValue;
        ...
        mapMarker.map = self;
        if ((NSDictionary*)array[@"icon"]) {
            mapMarker.icon = [self getMarkerImage:array];
        }
        if ((BOOL)array[@"active"] == false) {
            mapMarker.opacity = 0.1;
        } else {
            [path addCoordinate: mapMarker.position];
        }
    }
...
- (UIImage *)getMarkerImage:(NSDictionary *)marker
{
    ...
    if (!markerImages[markerPath]) {
        ...
        NSURL *markerUrl = [NSURL URLWithString:markerPath];
        
        NSURLRequest *request = [NSURLRequest requestWithURL:markerUrl];
        
        if([NSURLConnection canHandleRequest:request]) {
            markerImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:markerUrl]];
        } else {
            markerImage = [UIImage imageWithData:[NSData dataWithContentsOfFile:markerPath]];
        }
