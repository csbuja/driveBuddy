        ...
        GoogleMap.OnMyLocationButtonClickListener
{
    ...
    /**
     * Store bitmap descriptors to avoid rendering same icon multiple times
     */
    final HashMap<String, BitmapDescriptor> bitMapDescriptors = new HashMap<String, BitmapDescriptor>();
    ...
    private void addMapMarkers(GoogleMap googleMap) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (marker.hasKey("icon")) {
                ...
                markerWithCustomIcon(googleMap, latLng, iconMeta.getString("uri"), publicId);
            } else {
     ...
     * @param uri
     */
    private void markerWithCustomIcon(final GoogleMap googleMap, final LatLng latLng, String uri, final String publicId) {
        try {
            // Build Picasso target
            Target target = buildCustomTargetObject(googleMap, latLng, publicId);

            // Protect target from garbage collector
            protectedFromGarbageCollectorTargets.add(target);

            // Check if given URI is a resource identifier
            int resourceIdentifier = reactContext.getResources().getIdentifier(uri, "drawable", reactContext.getPackageName());
            ...
            if (resourceIdentifier == 0) {
                ...
                // Load icon from resource identifier
                Picasso.with(reactContext)
    ...
    /**
     * Builds a new Target class for Picasso
     *
     * @param googleMap
     * @param latLng
     * @param publicId
     * @return Target
     */
    private Target buildCustomTargetObject(final GoogleMap googleMap, final LatLng latLng, final String publicId)
    {
        return new Target() {
            @Override
            public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                ...
                // Check if bitmap has been already rendered
                BitmapDescriptor descriptor = bitMapDescriptors.get(publicId);

                if (descriptor == null) {
                    descriptor = BitmapDescriptorFactory.fromBitmap(bitmap);
                    bitMapDescriptors.put(publicId, descriptor);
                }
